...
Rprof(filename = here("ongoing_analysis/Mem&Time/data", paste0(now, "_250k_new", ".out")), append = FALSE, memory.profiling = TRUE)
...
profile <- summaryRprof(filename = here("ongoing_analysis/Mem&Time/data", paste0(now, "_250k_new", ".out")), chunksize = -1L, 
                        memory = "tseries", diff = FALSE)
max_mem <- max(rowSums(profile[,1:3]))*0.00000095367432

temp_table <- data.frame(250000, 1000, 0.005, max_mem)
