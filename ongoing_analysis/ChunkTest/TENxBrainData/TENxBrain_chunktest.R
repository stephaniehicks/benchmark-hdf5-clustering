...
if (mode == "time"){
  if(!file.exists(here("ongoing_analysis/ChunkTest/TENxBrainData/Output", file_name))){
    profile_table <- data.frame(matrix(vector(), 0, 12, 
                                     dimnames=list(c(), c("observations", "genes",
                                                          "batch size","abs batch size",
  ...
  if (method == "hdf5"){
    if (choice == "full"){
      ...
      temp_table <- data.frame(observations = dim(counts(tenx))[2], genes = dim(counts(tenx))[1], batch_size = batch, 
                               abs_batch_size =  round(dim(counts(tenx))[2]*batch, -1), 
                               time1 = time1[3], time2 = NA, time3 = NA, geometry = chunk, dimension_1 = seed(counts(tenx))@chunkdim[1], 
                               dimension_2 = seed(counts(tenx))@chunkdim[2], choice = "two step", label = paste0(chunk, "_", method))
      write.table(temp_table, file = here("ongoing_analysis/ChunkTest/TENxBrainData/Output", file_name), 
                  sep = ",", append = TRUE, quote = FALSE, col.names = FALSE, row.names = FALSE)
    }
  }
  if (method == "mbkmeans"){
    if (choice == "full"){
      ...
      invisible(mbkmeans(tenx_km, clusters=k, batch_size = as.integer(dim(counts(tenx))[2]*batch), num_init=1, max_iters=100,
                                                                      calc_wcss = FALSE, compute_labels=TRUE))
      time.end1 <- proc.time()
      time1 <- time.end1 - time.start1
      print(time1)
      ...
      time.start2 <- proc.time()
      output<- mbkmeans(tenx_km, clusters=k, batch_size = as.integer(dim(counts(tenx))[2]*batch), num_init=1, max_iters=100,
                        calc_wcss = FALSE, compute_labels=FALSE)
      time.end2 <- proc.time()
      time2 <- time.end2 - time.start2
