...
new_mem %>% 
  ...
  ggplot(aes(x = as.factor(abs.batch.size), y = memory, color = geometry)) +
          #geom_line()+
          geom_point()+
          labs(title = "HDF5 Geometry: Memory in Two Steps") + 
...
new_time %>% 
  filter(choice=="full") %>%
  dplyr::group_by(geometry, observations, abs.batch.size) %>% 
  ggplot(aes(x = as.factor(abs.batch.size), y = time1, color = geometry)) +
          #geom_line()+
          geom_point()+
...
```{r}
new_time %>% 
  gather("time_type", "time", -c(1:5,8:12))%>%
  ...
  ggplot(aes(x = as.factor(abs.batch.size), y = time, color = geometry)) +
          ...
          geom_point()+
          labs(title = "HDF5 Geometry: Time") + 
  facet_wrap(~observations+time_type)
```
