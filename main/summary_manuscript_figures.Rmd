...
p_ari_obs_line_abs <- acc_table_abs %>% 
  ...
  ggplot(aes(x = abs_batch, y = mean, color = method)) + #observations/1e3
      ...
      labs(title = "Accuracy (ARI) for increasing number of cells and absolute batch size on simulated data") + 
...
p_wcss_obs_line_abs <- acc_table_abs %>% 
  ...
  ggplot(aes(x = abs_batch, y = mean, color = method)) +
          ...
          geom_errorbar(aes(ymin=mean-sd, ymax=mean+sd), width=.2,
                position=position_dodge(0.05))+
     labs(title = "Accuracy (WCSS) for increasing number of cells and absolute batch size on simulated data") + 
        xlab("Number of cells (thousands)") + 
...
p_wcss_real_abs <- acc_real_table_abs %>% 
  ...
  ggplot(aes(x = abs_batch, y = mean, color = method)) +
          ...
          geom_errorbar(aes(ymin=mean-sd, ymax=mean+sd), width=.2,
                position=position_dodge(0.05))+
     labs(title = "Accuracy (WCSS) for increasing absolute batch size on real data") + 
