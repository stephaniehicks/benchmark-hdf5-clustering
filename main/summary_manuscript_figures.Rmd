...
sub_time_plot <- sub_time_table %>% 
  dplyr::filter(Algorithm == "k-means" | batch == 500, 
                machine == "solid state drive") %>%
  dplyr::group_by(Algorithm, ncells, batch_size) %>%
  dplyr::summarize(mean_elapsed = mean(elapsed/60)) %>%
...
```{r}
mem_table <- read.csv(here("ongoing_analysis/ChunkTest/TENxBrainData/Output/mem_davide_mac.csv"), header=TRUE, sep=",")
time_table <- read.csv(here("ongoing_analysis/ChunkTest/TENxBrainData/Output/time_davide_mac.csv"), header=TRUE, sep=",")
```

```{r}
p_mem <- mem_table %>% 
  dplyr::group_by(observations, label) %>% 
  ggplot(aes(x = observations, y = memory/1024, color=geometry)) +
          geom_point()+
          geom_line()+
...
p_time <- time_table %>% 
  ...
  dplyr::summarize(mean = mean(time1/60), 
                   sd = sd(time1/60)) %>% 
  ggplot(aes(x = observations, y = mean, color = label)) +
          geom_point()+
          geom_line()+
          geom_errorbar(aes(ymin=mean-sd, ymax=mean+sd), width=.2,
                position=position_dodge(0.05)) +
    labs(title = "THDF5 Geometry: Time") + 
    xlab("Observations")+
    ylab("Time(Minutes)")
