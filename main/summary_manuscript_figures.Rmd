...
acc_table_abs <- 
  cbind(rbind(acc_table_abs_davide, acc_table_abs_ruoxi), 
      "machine" = c(rep("solid state drive", nrow(acc_table_abs_davide)),
                    rep("hard disk drive", nrow(acc_table_abs_ruoxi))))

acc_table_abs <- acc_table_abs %>% 
  transform(normal_WCSS = WCSS/observations) %>% 
  dplyr::rename(Algorithm = method)
acc_table_abs$machine <- factor(acc_table_abs$machine, 
                                levels = c("solid state drive", "hard disk drive"))
...
acc_real_table_abs <- 
  cbind(rbind(acc_real_table_abs_davide, acc_real_table_abs_ruoxi[-5]), 
      "machine" = c(rep("solid state drive", nrow(acc_real_table_abs_davide)),
                    rep("hard disk drive", nrow(acc_real_table_abs_ruoxi))))
...
#  hard disk drive   450 450 450 450   0 450 450  450    0    0    0    0     0
#  solid state drive   0   0   6   6   5   6   1    6    7    8    6    6     6
```
...
In this figure we show the solid state drive 
results and show the hard disk drive results in supplemental. 
...
real scRNA-seq data. In this figure we show the solid state drive 
results and show the hard disk drive results in supplemental. 
...
p_wcss_real_abs <- acc_real_table_abs %>%   
  dplyr::filter(machine == "hard disk drive") %>% 
  dplyr::filter((observations %in% c(5000,10000,25000)))%>%
...
p2_abs
pdf(here::here("main", "figs", "fig-sims-and-real-accuracy.pdf"), width = 10, height =10)
print(p2_abs)
dev.off()
```
...
time_table_davide <- time_table_davide %>% 
  dplyr::rename(chunk_size = geometry)  %>% 
  mutate(chunk_size = 
           factor(chunk_size, levels = c("best", "default",
                                         "worst", "singleChunk")))
levels(time_table_davide$chunk_size) <- 
  c("per cell (best)", "default", "per gene (worst)", "single chunk")
```

load in ruoxi's mem and time results
```{r}
#mem
mem_table_ruoxi <- 
  read.csv(here("ongoing_analysis", "ChunkTest", "TENxBrainData", 
                "Output", "mem_ruoxi_cluster.csv"), header=TRUE, sep=",")
mem_table_ruoxi <- mem_table_ruoxi %>% 
  dplyr::rename(chunk_size = geometry)  %>% 
  mutate(chunk_size = 
           factor(chunk_size, levels = c("best", "default",
                                         "worst", "singleChunk")))
levels(mem_table_ruoxi$chunk_size) <- 
  c("per cell (best)", "default", "per gene (worst)", "single chunk")
...
time_table_ruoxi <- time_table_ruoxi %>% 
  dplyr::rename(chunk_size = geometry)  %>% 
  mutate(chunk_size = 
           factor(chunk_size, levels = c("best", "default",
                                         "worst", "singleChunk")))
levels(time_table_ruoxi$chunk_size) <- 
  c("per cell (best)", "default", "per gene (worst)", "single chunk")
```
...
Save the legend on its own
```{r}
my_legend <- cowplot::get_legend(p_mem)
```
...
p4 <- ggdraw() +
    ...
    draw_plot(p_mem, x = 0, y = 0.46, width = 1, height = .48) +
    draw_plot(p_time, x = 0, y = 0, width = 1, height = .48) +
    draw_plot_label(label = LETTERS[1:2], size = 25,
...
p_ari_obs_line_abs_all <- acc_table_abs %>% 
  dplyr::filter(machine == "solid state drive") %>% 
  dplyr::group_by(Algorithm, observations, abs_batch) %>% 
  dplyr::summarize(mean = mean(ARI), 
                   sd = sd(ARI)) %>% 
  ggplot(aes(x = abs_batch, y = mean, color = Algorithm)) +
