...
p_wcss_real_abs <- acc_real_table_abs %>%   
  #dplyr::filter((method %in% c("kmeans, mbkmeans")))%>%
  #dplyr::filter((method != "hdf5")) %>%
  dplyr::group_by(observations, method, abs_batch) %>% 
  mutate(q85 = quantile(normal_WCSS, 0.85), row = row_number())%>% 
  dplyr::filter(normal_WCSS <= q85) %>%
...
for (i in (1:length(files_vark))){
  ...
  vark_table <- rbind(vark_table, temp_table)
}
...
```{r, warning=FALSE}
p_vark_ari <- vark_table %>% 
  dplyr::filter((observations == 25000)) %>%
  #dplyr::filter((method == "kmeans")) %>%
  dplyr::group_by(method, observations, batch_size,k) %>% 
  dplyr::summarize(mean = mean(ARI), 
                   sd = sd(ARI)) %>% 
  ggplot(aes(x = k, y = mean, color = method)) +
          ...
          geom_errorbar(aes(ymin=mean-sd, ymax=mean+sd), width=.2,
                position=position_dodge(0.05))+
     labs(title = "Accuracy (ARI) for increasing number of K using simulated data") + 
        xlab("Number of K") + 
        ylab("ARI") + 
  theme(legend.position = "top", legend.justification= "center") +
  facet_wrap(~observations+batch_size, scales = "free") 
p_vark_ari
```
...
p_vark <- vark_table %>% 
  ...
  ggplot(aes(x = k, y = mean, color = method)) +
          ...
          geom_errorbar(aes(ymin=mean-sd, ymax=mean+sd), width=.2,
                position=position_dodge(0.05))+
     labs(title = "Accuracy (WCSS) for increasing number of K using simulated data") + 
        xlab("Number of K") + 
        ylab("Normalized WCSS") + 
  theme(legend.position = "top", legend.justification= "center") +
  facet_wrap(~observations+batch_size, scales = "free") 
