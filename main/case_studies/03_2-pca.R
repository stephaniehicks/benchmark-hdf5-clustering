...
out_name <- paste0(data_name,"_step3_pca_", now, ".out")
...
#keep top 30% most variable genes
for_pca <- t(logcounts(sce)[names(vars)[1:as.integer(length(names(vars))*0.3)],])

#perform pca
Rprof(filename = here("main/case_studies/output/Memory_output", paste0(out_name)), append = FALSE, memory.profiling = TRUE)
...
profile <- summaryRprof(filename = here("main/case_studies/output/Memory_output", paste0(out_name)), chunksize = -1L, 
...
h5File <- here("main/case_studies/data/pca", data_name, paste0(data_name, "_pca.h5"))
h5createFile(h5File)
