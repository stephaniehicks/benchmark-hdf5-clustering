...
rx_files <- list.files(here("main", "case_studies", "output"), 
                          pattern = "_ruoxi_cluster.csv")
```
...
Read in files from `_ruoxi_cluster.csv`
```{r}
colnames_memory <- c("dataset", "run", "ncells", "ngenes", "step", 
                   "method", "batch_prop", "B", "max_mem")
colnames_time <- c("dataset", "run", "ncells", "ngenes", "step", 
...
analysis_type <- c("memory", "time")
pef_results <- vector("list", length(analysis_type))
for(ty in analysis_type){
  tmp <- rx_files[grep(ty, rx_files)] %>% 
            map(read_csv, col_names = FALSE) %>% 
            bind_rows()
...
Convert three datasets from `_ruoxi_cluster.csv` into one long dataset
...
pef_results$time <- 
  ...
  tidyr::pivot_longer(cols = -c(dataset:B), 
                      names_to = "time_type", 
                      values_to = "time")
rx_results <- dplyr::left_join(pef_results$time, pef_results$memory)

rx_results$dataset <- 
  forcats::fct_reorder(rx_results$dataset, rx_results$ncells, .desc = FALSE)
rx_results$batch_prop <- factor(rx_results$batch_prop)
rx_results

final_results <- rx_results
```
