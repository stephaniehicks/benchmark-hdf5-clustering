...
if(!file.exists(here("main/case_studies/data/full/TENxBrainData/SingleR_Allen_10X_V2_labels.rds"))) {
  
#  if(!dir.exists((here("main/case_studies/data/full/Allen_10X_V2")))) {
    if(!file.exists((here("main/case_studies/data/full/TENxBrainData/allen_10x.h5")))) {
    ...
    se_allen <- SingleCellExperiment(assays = list(counts=mat[,which_cell]),
                                     colData = anno,
                                     rowData = features)
#    saveHDF5SummarizedExperiment(se_allen, dir = here("main/case_studies/data/full/Allen_10X_V2"))
#  } else {
#    se_allen <- loadHDF5SummarizedExperiment(here("main/case_studies/data/full/Allen_10X_V2"))
#  }
  ...
  rownames(se_allen) <- rowData(se_allen)[,2]
  rownames(allen_pseudo) <- rowData(se_allen)[,2]
  
  pred <- SingleR(test=sce_pseudo, ref=allen_pseudo,
                  labels=se_allen$subclass_label,
                  method = "cluster",
                  clusters = sce$mbk_15,
                  assay.type.test = "sum",
                  assay.type.ref = "sum")
  ...
  saveRDS(pred, file = here("main/case_studies/data/full/TENxBrainData/SingleR_Allen_10X_V2_labels.rds"))
} else {
  pred <- readRDS(file = here("main/case_studies/data/full/TENxBrainData/SingleR_Allen_10X_V2_labels.rds"))
}
...
clusters <- inner_join(clusters, cluster_anno[,3:4], by = c("celltype"="subclass_label")) %>% 
  unique %>%
  dplyr::arrange(label) 
...
tmp = clusters$class_label
names(tmp) = clusters$label
sce$class <- plyr::revalue(factor(sce$mbk_15), replace = tmp)

plot_hexbin_meta(sce, col="class", action = "majority",
                 colors = clusterExperiment::bigPalette)
