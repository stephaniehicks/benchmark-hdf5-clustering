...
I-1. New results (seperating simulation and test)
```{r}
...
I-2. Compare new results with before-patch results
```{r, warning=FALSE}
p_mem_patch_hdf5 <- mem_table_total %>%
  dplyr::filter(!(method %in% c("kmeans-old", "kmeans-new","mbkmeans-old","mbkmeans-new" )))%>%
 # dplyr::filter(!(observations %in% c(1000000)))%>%
  dplyr::filter((observations %in% c(1000, 5000, 25000, 75000, 350000, 500000)))%>%
  dplyr::group_by(method, observations, batch_size_perc) %>% 
  dplyr::summarize(mean = mean(memory), 
                   sd = sd(memory)) %>% 
  ggplot(aes(x = observations/1e3, y = mean, color = method)) +
...
p_mem_patch_mbkm <- mem_table_total %>%
 ...
 # dplyr::filter(!(observations %in% c(1000000)))%>%
  dplyr::filter((observations %in% c(1000, 5000, 25000, 75000)))%>%
  dplyr::group_by(method, observations, batch_size_perc) %>% 
  dplyr::summarize(mean = mean(memory), 
                   sd = sd(memory)) %>% 
  ggplot(aes(x = observations/1e3, y = mean, color = method)) +
...
p_elp_time_obs_total_2 <- time_table_total2 %>%
  dplyr::filter(!(method %in% c("kmeans", "kmeans-old", "mbkmeans")))%>%
  #dplyr::filter(!(observations %in% c(500000,1000000, 100000, 175000, 250000)))%>%
  dplyr::group_by(method, observations, batch_size_perc) %>% 
  dplyr::summarize(mean = mean(elapsed_time), 
                   sd = sd(elapsed_time)) %>% 
  ...
  mutate(method = factor(method, levels = c("kmeans", "mbkmeans", "hdf5")), 
         batch_size_perc = factor(scales::percent(acc_table$batch_size), 
                             levels = c("0.5%", "1.0%", "5.0%", 
                                        "10.0%", "20.0%","35.0%", "50.0%","80.0%", "100.0%")))
...
```
```{r, eval=FALSE}
p_ari_obs_line_small <- acc_table %>% 
  dplyr::filter(observations %in% c(500,1000,2000,4000,5000,6000,8000,10000))%>%
  dplyr::group_by(method, observations, batch_size_perc) %>% 
  dplyr::summarize(mean = mean(ARI), 
                   sd = sd(ARI)) %>% 
  ggplot(aes(x = observations/1e3, y = mean, color = method)) +
...
```
```{r, warning=FALSE, eval=FALSE}
p_wcss_obs_line_small <- acc_table %>% 
  dplyr::filter(observations %in% c(500,1000,2000,4000,5000,6000,8000,10000))%>%
  dplyr::group_by(method, observations, batch_size_perc) %>% 
  dplyr::summarize(mean = mean(WCSS), 
                   sd = sd(WCSS)) %>% 
