...
p_acc_k_kplus <- k_acc_table %>%
  ...
  ggplot(aes(x = k, y = mean, color = method)) +
        ...
        ylab("ARI") + 
  facet_wrap(method~batch_size_perc, scales = "free") + 
  theme(legend.position = "top", legend.justification= "center") +
  scale_color_discrete(name = "Method")
...
real_total <- merge(real_time, real_mem, by=c("Data","Cells","Genes","Step","Method","B"))
#cbind.fill(real_time, real_mem)

#select variables to be put into the summary table
df <- real_total %>%
  select(Data = Data,
         Step = Step,
         Method = Method,
...
df %>%
  group_by(Data, Step, Method, B) %>% 
  summarize(mean = mean(Elapsed_time),
            sd = sd(Elapsed_time))


df_sum <- CreateTableOne(data = df, strata = c("Data","Step","Method","B"))
```

```{r}
real_time %>% 
    tab_cells(Elapsed_time) %>%
    tab_cols(Data) %>% 
    tab_stat_mean_sd_n() %>%
