...
final_table <- final_table %>% 
  mutate(method = factor(method, levels = c("kmeans", "mbkmeans", "hdf5")), 
         batch_size = factor(scales::percent(final_table$batch_size), 
                             levels = c("0.5%", "1.0%", "20.0%", "50.0%", "80.0%", "100.0%")))
levels(final_table$method) <- c("kmeans", "mbkmeans", "mbkmean_hdf5")
levels(final_table$batch_size) <- levels(final_table$batch_size)[c(1,2,4:6,3)]
```
...
```{r, eval=FALSE}
pdf(here("main", "figs", "memory_obs.pdf"), width = 8, height=6)
print(p1)
dev.off()
```
