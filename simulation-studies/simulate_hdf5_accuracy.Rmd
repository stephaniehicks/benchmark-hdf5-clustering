...
bench_hdf5 <- function(i, n_cells, 
    ...
    if(method == "kmeans"){
      ...
      htmlwidgets::saveWidget(memory_compare,"memory_compare_kmeans.html")
      
      outputlist <- list(sim_data = sim_data, 
    ...
    if(method == "mbkmeans"){
      ...
      htmlwidgets::saveWidget(memory_compare,"memory_compare_mbkmeans.html")
    ...
    if(method == "hdf5"){
      ...
      htmlwidgets::saveWidget(memory_compare,"memory_compare_hdf5.html")
...
bench_hdf5 <- function(i, n_cells, 
    ...
    # this function needs to be adjusted to point to the HDF5 matrix on disk
    if(method == "hdf5"){
      Rprof("hdf5_2.out", append = TRUE, memory.profiling = TRUE)
      mbkmeans_hdf5_time <- system.time(mbkmeans_hdf5_output <- mbkmeans::mini_batch(
              sim_data_hdf5, clusters = k_centers, 
              batch_size = batch_size, num_init = num_init, 
              max_iters = max_iters, init_fraction = init_fraction,
     ...
     Rprof(NULL)
      ...
      outputlist <- list(sim_data = sim_data, 
        mbkmeans_hdf5_output = mbkmeans_hdf5_output,
        mbkmeans_hdf5_time = mbkmeans_hdf5_tim)    
    }
...
#batch_percent <- 0.001
batch_percent <- 0.25
save_bs_0001_km <- mclapply(seq_len(B),
...
save_bs_0001_mbkm <- mclapply(seq_len(B),
                  n_cells = nC, n_genes = nG, 
                  bench_hdf5, k_centers = 3,
                  batch_size = nC*batch_percent, num_init = 10, max_iters = 100, 
                  init_fraction = .1, initializer = mbkmeans_initializer, 
...
save_bs_0001_hdf5 <- mclapply(seq_len(B),
                  n_cells = nC, n_genes = nG, 
                  bench_hdf5, k_centers = 3,
                  batch_size = nC*batch_percent, num_init = 10, max_iters = 100, 
                  init_fraction = .1, initializer = mbkmeans_initializer, 
...
htmlwidgets::saveWidget(memory_compare,"memory_compare2.html")
...
object.size(sim_data)
object.size(sim_data_hdf5)

memory_compare <- profvis({
  
km_test <- stats::kmeans(sim_data$obs_data, centers=3)
...
htmlwidgets::saveWidget(memory_compare,"memory_compare_test.html")

typeof(sim_data_hdf5)
object.size(hdf5_test)
object.size(hdf5_test)
object.size(mbkmeans_test)
```

```{r}
sim_data <- simulate_gauss_mix(n_cells=1000, n_genes=500)
sim_data_hdf5 <- as(as.matrix(sim_data$obs_data),"HDF5Matrix")

Rprof("kmeans2.out", append = TRUE, memory.profiling = TRUE)
km_test <- stats::kmeans(sim_data$obs_data, centers=3)
Rprof(NULL)
summaryRprof("kmeans2.out", memory = "both")
